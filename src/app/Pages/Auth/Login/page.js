...
export default function LoginPage() {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const result = await dispatch(login({ email, password, userRole: 'Admin' })).unwrap();
      if (result.success) {
      ...
      <div className="w-full md:w-1/2 flex flex-col justify-center items-center p-8 bg-white">
        <div className="w-full max-w-md">
          ...
          <div className="mt-4 text-center text-sm text-gray-600">
            Customer Login?{' '}
            <Link href="/Pages/Auth/CustomerLogin" className="text-indigo-600 font-medium hover:text-indigo-500">
              Login
            </Link>
          </div>
        </div>
